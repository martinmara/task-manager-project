...
/**
 * Admin check (works with:
 * - props.user.role (single role relation)
 * - props.user.roles (many-to-many)
 * - role name stored as "Admin" or "admin"
 */
...
const safeTasks = computed(() => {
  ...
  if (!u) return [];
  return isAdmin.value ? all : all.filter(t => t?.user_id === u.id);
});
  ...
  for (let i = 0; i < totalBoxes; i++) {
    ...
    if (dateNum > 0 && dateNum <= daysInMonth) {
      ...
      const tasksForDay = safeTasks.value.filter(t => {
        ...
        return String(t.due_date).slice(0, 10) === fullDate;
      });
...
<template>
  <AuthenticatedLayout :user="user" :tasks="tasks" :teams="teams" :projects="projects">
    <div class="relative z-10 p-6 space-y-8 text-gray-100">
      <!-- Header -->
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          <section>
            ...
            <div v-if="safeTasks.length" class="grid md:grid-cols-2 gap-4">
              <div
              ...
              >
                ...
                <div class="mt-3 text-xs flex items-center gap-2">
                  <span class="inline-block w-2 h-2 rounded-full bg-yellow-400/70"></span>
                  <span>{{ task.project?.name ?? 'No Project' }}</span>
                </div>
              </div>
            </div>

            <div v-else class="text-sm text-gray-200/80">
              No tasks yet.
            </div>
          </section>
        </div>
        ...
        <div class="space-y-6">
          ...
          <section class="bg-white/20 p-5 rounded-2xl shadow-lg">
            <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2">
              <ClipboardList class="w-6 h-6 text-yellow-400" />
              Projects
            </h2>

            <div v-if="safeProjects.length" class="space-y-3">
              <div
          ...
          <section class="bg-white/20 p-5 rounded-2xl shadow-lg">
            ...
            <div v-if="safeTeams.length" class="space-y-3">
              <div
      ...
      <section class="bg-white/20 p-5 rounded-2xl shadow-lg mt-10">
        ...
        <div class="grid grid-cols-7 text-sm font-semibold mb-2 text-gray-200">
          <div v-for="d in ['Sun','Mon','Tue','Wed','Thu','Fri','Sat']" :key="d" class="text-center">
            {{ d }}
          </div>
        </div>
      ...
      <!-- Optional quick indicator (helps debugging / can remove later) -->
      <div class="text-xs text-gray-200/70">
        Role: {{ isAdmin ? 'Admin' : 'User' }} • Teams: {{ safeTeams.length }} • Projects: {{ safeProjects.length }}
      </div>
    </div>
  </AuthenticatedLayout>
</template>
